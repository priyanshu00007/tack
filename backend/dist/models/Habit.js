...
habitSchema.virtual('completionRate').get(function () {
    ...
    const recentCompletions = this.completedDates.filter(date => new Date(date) >= thirtyDaysAgo);
...
habitSchema.virtual('isCompletedToday').get(function () {
    ...
    return this.completedDates.some(date => new Date(date).toDateString() === today);
});
...
habitSchema.virtual('weekCompletions').get(function () {
    ...
    return this.completedDates.filter(date => new Date(date) >= weekStart).length;
});
...
habitSchema.methods.toggleToday = function () {
    const today = new Date();
    const todayStr = today.toDateString();
    const existingIndex = this.completedDates.findIndex(date => new Date(date).toDateString() === todayStr);
    if (existingIndex > -1) {
...
habitSchema.statics.getTodayHabits = function (userId) {
    return this.find({ userId, isActive: true }).sort({ createdAt: 1 });
};
